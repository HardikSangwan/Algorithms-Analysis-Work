...
def dis_euc(x_i,y_i,x_i_1,y_i_1):
    xd = abs(x_i_1)-abs(x_i)
    yd = abs(y_i_1)-abs(y_i)
...
class MSTApprox:
    ...
    def rdata(self):
        lineOut = []
        with open('./DATA/'+self.location+'.tsp') as file1:
            next(file1)
            for l in file1:
                a = l.split()
                if "TYPE:" in a[0]:
    ...
    def wdata(self,out,tot):
        tot = (str)((int)(tot))
        with open('Output/'+self.location+ "_MSTApprox_" + str(self.timelimit)+'.sol','w') as file1:
            file1.write(tot)
            file1.write('\n')
            for (x,y,z) in out:
    ...
    def wtrace(self,time,tot):
        with open('Output/'+self.location + "_MSTApprox_" + str(self.timelimit) +'.trace','w') as file1:
            file1.write('{:.2f} {}\n'.format(time, tot))
