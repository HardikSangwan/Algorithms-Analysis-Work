...
def switchnodes(a,dim):
    n = dim-1
    i = random.randint(1,n)
    j = random.randint(1,n)
    a_org=[[0]*3 for _ in xrange(dim+1)]
    a0=a[i]
    a1 = a[j]
...
def tsp_sa(filepath,co_time,seed):
    ...
    while  T>T_min and elapsed < co_time:
        ...
        output_trace.write(str(cbest) + ","+ str(elapsed)+"\n")
...
#    seed = sys.argv[4]
#    seed = int(seed[0])
    tourcost = [[0]*10 for _ in xrange(14)]
    tourcost_avg = [0]*14
    elapsed_avg = [0]*14
    elapsed = [[0]*10 for _ in xrange(14)]
